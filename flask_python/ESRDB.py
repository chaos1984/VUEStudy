...
class ESR(db.Model):
    ...
    ENO = db.Column(db.String(80), nullable=False)
    ...
    Lock_Mat = db.Column(db.String(80), nullable=False)
    ...
    Hinge_HLratio= db.Column(db.Float(), nullable=False)
    ...
    Res = db.Column(db.String(800), nullable=False)
    Remarks = db.Column(db.String(800), nullable=False)
    Originator = db.Column(db.String(80), nullable=False)
    Log = db.Column(db.String(80), nullable=False)
    AI = db.Column(db.String(80), nullable=False)
    def __repr__(self):
        return "{'ID':'%s','OEM':'%s','PRJ':'%s' ,'AFIS':'%s','ENO':'%s','PE':'%s','CAE':'%s','Interface':'%s','CV_Mat':'%s','H_Mat':'%s','Lock_Mat':'%s','Tearline':'%s','E_Mat':'%s','Inflator':'%s','C_Mat':'%s','C_Type':'%s','C_Diam':'%s','C_Tether':'%s','H_Width':'%s','Flappy_Mass':'%s','Wrapper':'%s','Hinge_Area':'%s','Hinge_Width':'%s','Hinge_HLratio':'%s','H_Plane':'%s','H_Neck':'%s','UnderCut':'%s','Simulation':'%s','Testing':'%s','DateRange':'%s','CV_Height':'%s','CV_Leather':'%s','Res':'%s','Remarks':'%s','Originator':'%s','Log':'%s','AI':'%s'}" \
            % (str(self.ID),self.OEM,self.PRJ,self.AFIS,self.ENO,self.PE,self.CAE,self.Interface,self.CV_Mat,self.H_Mat,self.Lock_Mat,self.Tearline,self.E_Mat,self.Inflator,self.C_Mat,self.C_Type,self.C_Diam,self.C_Tether,self.H_Width,self.Flappy_Mass,self.Wrapper,self.Hinge_Area,self.Hinge_Width,self.Hinge_HLratio,self.H_Plane,self.H_Neck,self.UnderCut,self.Simulation,self.Testing,self.DateRange,self.CV_Height,self.CV_Leather,self.Res,self.Remarks,self.Originator,self.Log,self.AI)
